    ...
    if (value === null || value === undefined) {
      ...
      if (value === '' ) { // This is here for support of legacy code
        ...
        try {
          ...
          throw new Error('Expected ' + f.name + ' type to be a Date string | Empty string.');
        }
      }
    } else if (typeof value === 'number') {
      throw new Error('Expected ' + f.name + ' type to be a null | undefined | Date | Date string | Empty string.');
    } else {
      try {
        value = new Date(value);
        return value.toISOString();
