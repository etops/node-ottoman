    ...
    Object.defineProperty(this, 'bucket', {
        get: function () {
            if (this.store instanceof StoreAdapter.Couchbase) {
                ...
                throw new Error('Cannot access bucket property after non-Couchbase store set.');
            }
        },
        set: function (bucket) {
...
/**
 * Returns the type def for a given model by name and namespace.
 *
 * @param {string} type
 * @param {string} namespace
 * @returns {ModelInstance|TypeDef|CoreType}
 */
Ottoman.prototype.typeByNameAndNamespace = function (type, namespace) {
    var coreType = Schema.coreTypeByName(type);
    if (coreType && coreType.namespace === namespace) {
        return coreType;
    }
    var model = this.models[type];
    if (model && model.namespace === namespace) {
        return model;
    }
    var typedef = this.types[type];
    if (typedef && typedef.namespace === namespace) {
        return typedef;
    }
    return null;
};
 ...
 *  @param {string} options.namespaceKey
...
Ottoman.prototype._createSchema = function (name, schemaDef, options) {
    ...
    if (options.namespaceKey) {
        schema.namespaceKey = options.namespaceKey;
    }
    if (options.store) {
...
Ottoman.prototype._findModelsByQuery =
    function (mdlInst, model, query, callback) {
        ...
        this._findModelsByDefIndex(query.type, remoteType, [query.field], [mdlInst.id()], options, callback);
    };
/**
...
Ottoman.prototype._buildAndRegisterModel = function (name, schemaDef, options) {
    if (options && options.namespace) {
        if (this.typeByNameAndNamespace(name, options.namespace)) {
            throw new Error('A model with this namespace and ' +
...
Ottoman.prototype._ensureModelIndices = function (model, callback) {
    ...
    for (var i = 0; i < schema.indices.length; ++i) {
        ...
        if (!(index instanceof Schema.RefDocIndex)) {
            ...
            console.log('Push model index', { type: index.type, name: idxName, fields: fields });
            modelIndices.push({
                type: index.type,
                name: idxName,
                fields: fields
    ...
    for (var k = 0; k < modelIndices.length; ++k) {
        ...
        schema.store.createIndex(modelIndex.type, schema.namePath(false), modelIndex.name, modelIndex.fields, handler);
    }
};
/**
...
Ottoman.prototype.getModel = function (name, prefixed) {
    if (prefixed) {
        ...
        if (namePrefix !== prefix) {
            throw new Error('Model does not have the expected prefix ' +
                '(`' + namePrefix + '` != `' + prefix + '`).');
        }
...
//# sourceMappingURL=ottoman.js.map